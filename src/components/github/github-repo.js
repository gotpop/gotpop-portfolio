  ...
  return (
    <div className="bg-white p-4 rounded" key={index}>
      <div className="flex justify-between mb-4">
        ...
        <Link
          href={repo.html_url}
          className="flex items-center justify-between px-6 py-2 border-2 border-gray-600 text-gray-800 font-medium text-xs leading-tight rounded bg-gray-100 hover:bg-black hover:bg-opacity-5 focus:outline-none focus:ring-0 transition duration-150 ease-in-out">
          View repo on GitHub
      ...
      <div className="flex flex-wrap gap-1 mb-4">
        {repo.topics.map((topic, index) => {
          const list = (
            <span
              key={index}
              className="block-inline rounded bg-gray-100 py-1 px-4 text-sm">
              {topic}
            </span>
          );
          return list;
